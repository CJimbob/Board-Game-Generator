    ...
    <main className="landing-shell">
      ...
      <section className="entry-panel" aria-label={text("进入游戏", "Enter game")}>
        ...
        <div className="setup-grid">
          ...
          <label>{text("电脑对手", "AI opponents")}<select value={botCount} onChange={(event) => setBotCount(Number(event.target.value))}>{Array.from({ length: 8 }, (_, count) => <option key={count} value={count}>{count === 0 ? text("不添加", "None") : text(`${count} 位${count === 1 ? "（推荐）" : ""}`, `${count}${count === 1 ? " (recommended)" : ""}`)}</option>)}</select></label>
        </div>
        ...
        {error && <p className="form-error" role="alert">{error}</p>}<p className="fine-print">{text("无需注册。服务器保存牌局；恢复码可让你在另一台设备返回座位。", "No registration required. The server saves the game; your recovery code lets you return from another device.")}</p>
      </section>
    </main>
  );
}
