  ...
  return (
    <main className="landing-shell">
      ...
      <section className="entry-panel" aria-label={text("进入游戏", "Enter game")}>
        ...
        {error && <p className="form-error" role="alert">{error}</p>}<button className="history-entry-button" onClick={onOpenHistory}>♜ {text("查看历史对局", "View match history")}</button><Link className="game-hub-link" href="/"><span>⌂</span><div><strong>{text("返回双游戏大厅", "Back to the game hall")}</strong><small>{text("切换至《六境争霸》或重新选择游戏", "Switch to The Six Realms or choose again")}</small></div><b>→</b></Link><p className="fine-print">{text("无需注册或模型密钥。策略电脑由服务器按规则运行；完成的对局会保存到王城档案。", "No account or model key required. Strategy AI follows the rules on the server; completed matches are saved in the city archive.")}</p>
      </section>
    </main>
  );
}
...
function Lobby({ game, act }: { game: Game; act: Act }) {
  ...
  async function shareInvite() {
    const url = `${window.location.origin}/crown-city?room=${game.code}`;
...
function Turns({ game, act }: { game: Game; act: Act }) {
  ...
  return <div className="turn-layout"><section className="table-area"><div className="turn-banner"><div><span>{text(`第 ${game.round} 轮 · 正在叫号 ${game.currentRank}`, `Round ${game.round} · Calling rank ${game.currentRank}`)}</span><h2>{myTurn ? text("轮到你行动", "Your turn") : text(`${active?.name ?? "玩家"} 正在行动`, `${active?.name ?? "Player"} is taking a turn`)}</h2></div>{myTurn && role && <div className={`my-role color-${role.color}`}><b>{role.rank}</b><span>{role.name}<small>{role.description}</small></span></div>}</div>
    ...
    {!myTurn && <><div className="spectator-message"><span>♜</span><p>{text("观察城市变化，等待你的角色被叫到。", "Watch the cities change while waiting for your character to be called.")}</p></div><section className="hand-section readonly-hand waiting-hand" aria-label={text("你的手牌", "Your hand")}><div className="section-title"><h3>{text("你的城区牌", "Your district cards")}</h3><span>{text("别人行动时可查看；轮到你时才能建造", "View while others act; build only during your turn")}</span></div>{me.hand.length ? <div className="card-row">{me.hand.map((district) => <DistrictCard key={district.uid} district={district} />)}</div> : <div className="empty-hand">{text("你暂时没有城区牌。", "You have no district cards.")}</div>}</section></>}
