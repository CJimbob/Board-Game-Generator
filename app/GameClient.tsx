  ...
  return (
    <main className="landing-shell">
      ...
      <section className="entry-panel" aria-label={text("进入游戏", "Enter game")}>
        ...
        {error && <p className="form-error" role="alert">{error}</p>}<button className="history-entry-button" onClick={onOpenHistory}>♜ {text("查看历史对局", "View match history")}</button><a className="game-hub-link" href="/"><span>⌂</span><div><strong>{text("返回双游戏大厅", "Back to the game hall")}</strong><small>{text("切换至《六境争霸》或重新选择游戏", "Switch to The Six Realms or choose again")}</small></div><b>→</b></a><p className="fine-print">{text("无需注册或模型密钥。策略电脑由服务器按规则运行；完成的对局会保存到王城档案。", "No account or model key required. Strategy AI follows the rules on the server; completed matches are saved in the city archive.")}</p>
      </section>
    </main>
  );
}
