  ...
  return (
    <main className="landing-shell">
      ...
      <section className="entry-panel" aria-label={text("进入游戏", "Enter game")}>
        ...
        {error && <p className="form-error" role="alert">{error}</p>}<button className="history-entry-button" onClick={onOpenHistory}>♜ {text("查看历史对局", "View match history")}</button><a className="realms-entry-link" href="/realms"><span>✦</span><div><strong>{text("进入《六境争霸》", "Enter The Six Realms")}</strong><small>{text("完整第二版式版图战争 · 3–6 人", "Complete second-edition-style realm war · 3–6 players")}</small></div><b>→</b></a><p className="fine-print">{text("无需注册或模型密钥。策略电脑由服务器按规则运行；完成的对局会保存到王城档案。", "No account or model key required. Strategy AI follows the rules on the server; completed matches are saved in the city archive.")}</p>
      </section>
    </main>
  );
}
