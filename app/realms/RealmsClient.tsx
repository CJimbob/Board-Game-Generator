...
function PlanningPanel({ game, me, language, act }: { game: Game; me: Player; language: Language; act: (action: string, payload?: Record<string, unknown>) => void }) {
  ...
  return <section className="action-card planning-card"><h3>{text("为有部队的区域下令", "Assign orders to occupied areas")}</h3><p>{text(`王庭允许 ${starLimit} 枚星级命令；本轮须放置 ${required} 枚。`, `Court position allows ${starLimit} starred orders; place ${required} orders this round.`)}</p><div className="order-area-tabs">{areas.map((area) => <button key={area.key} className={selected === area.key ? "selected" : ""} onClick={() => setSelected(area.key)}>{areaName(game, area.key, language)}{orders[area.key] ? ` · ${ORDER_LABELS[orders[area.key]][language === "zh" ? 0 : 1]}` : ""}</button>)}</div><div className="order-grid">{(Object.keys(game.orderCounts) as Order[]).map((order) => { const disabled = Boolean(forbidden(order)) || (usedCounts[order] ?? 0) >= game.orderCounts[order] && orders[selected] !== order || order.endsWith("_star") && Object.values(orders).filter((item) => item.endsWith("_star")).length >= starLimit && orders[selected] !== order; return <button key={order} className={orders[selected] === order ? "selected" : ""} disabled={disabled} onClick={() => setOrders((current) => { const next = { ...current }; if (!next[selected] && Object.keys(next).length >= required) delete next[Object.keys(next)[0]]; next[selected] = order; return next; })}>{ORDER_LABELS[order][language === "zh" ? 0 : 1]} <small>{usedCounts[order] ?? 0}/{game.orderCounts[order]}</small></button>; })}</div><button className="realm-primary" disabled={Object.keys(orders).length !== required} onClick={() => act("submitOrders", { orders })}>{text("封存全部命令", "Lock all orders")}</button></section>;
}
