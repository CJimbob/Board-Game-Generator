...
function RealmMap({ game, language, inspectedKey, actionKey, onInspect, selectableAreaIds }: { game: Game; language: Language; inspectedKey: string; actionKey: string; onInspect: (areaId: string) => void; selectableAreaIds: string[] }) {
  ...
  const renderContents = (definition: AreaDefinition, inPort = false) => {
    ...
    const [displayX, displayY] = rotateMapPoint([definition.x, definition.y]);
    const orderText = state.order === "hidden" ? (language === "zh" ? "密令" : "Sealed") : state.order ? ORDER_LABELS[state.order as Order][language === "zh" ? 0 : 1] : "";
    const orderTitle = state.order === "hidden" ? (language === "zh" ? "已放置密令 · 全员封存后揭示" : "Sealed order · revealed after everyone locks") : orderText;
    return <span className="area-content" style={{ left: `${displayX}%`, top: `${displayY}%` }} role={inPort ? undefined : "button"} tabIndex={inPort ? undefined : 0} onClick={inPort ? undefined : (event) => { event.stopPropagation(); chooseArea(definition.key); }} onKeyDown={inPort ? undefined : (event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); chooseArea(definition.key); } }}>
      ...
      <span className="area-icons">
        {definition.castle && <i title={language === "zh" ? `城堡 ${definition.castle}` : `Castle ${definition.castle}`}><b>{language === "zh" ? "城堡" : "Castle"}</b><span>{definition.castle}</span></i>}
        {definition.supply && <i title={language === "zh" ? `补给 ${definition.supply}` : `Supply ${definition.supply}`}><b>{language === "zh" ? "补给" : "Supply"}</b><span>{definition.supply}</span></i>}
        {definition.power && <i title={language === "zh" ? `王冠 ${definition.power}` : `Crown ${definition.power}`}><b>{language === "zh" ? "王冠" : "Crown"}</b><span>{definition.power}</span></i>}
        {state.blocked && <i className="blocked"><b>{language === "zh" ? "禁用" : "Blocked"}</b></i>}
        {state.neutral && !state.blocked && <i className="neutral" title={language === "zh" ? `中立兵力 ${state.neutral}` : `Neutral force ${state.neutral}`}><b>{language === "zh" ? "中立" : "Neutral"}</b><span>{state.neutral}</span></i>}
        {state.garrison && !state.neutral && <i className="garrison" title={language === "zh" ? `驻军 ${state.garrison}` : `Garrison ${state.garrison}`}><b>{language === "zh" ? "驻军" : "Garrison"}</b><span>{state.garrison}</span></i>}
      </span>
      ...
      {state.order && <i className={`map-order ${state.order === "hidden" ? "hidden" : ""}`} title={orderTitle} aria-label={orderTitle}>{orderText}</i>}
    </span>;
  };
  ...
  return <div className={`realm-map-shell ${expanded ? "expanded" : ""} ${showAllLabels ? "show-all-labels" : "compact-labels"}`}>
    ...
    {selectedDefinition && selectedState && <section className="map-selection-dock" style={{ "--owner": selectedOwner?.color ?? "#9a8d70" } as CSSProperties}>
      ...
      <div className="map-selection-status">{selectedState.units.length > 0 && <div className="map-inspector-units">{selectedState.units.map((unit) => <MapUnitPiece key={unit.id} unit={unit} color={factionMap.get(unit.faction)?.color} language={language} />)}</div>}<div className="map-resource-chips">{selectedDefinition.castle && <i>{language === "zh" ? "城堡" : "Castle"} {selectedDefinition.castle}</i>}{selectedDefinition.supply && <i>{language === "zh" ? "补给" : "Supply"} {selectedDefinition.supply}</i>}{selectedDefinition.power && <i>{language === "zh" ? "王冠" : "Crown"} {selectedDefinition.power}</i>}{selectedState.neutral && <i>{language === "zh" ? "中立兵力" : "Neutral force"} {selectedState.neutral}</i>}{selectedState.garrison && <i>{language === "zh" ? "驻军" : "Garrison"} {selectedState.garrison}</i>}{selectedState.order && <i className={selectedState.order === "hidden" ? "sealed-order-chip" : ""}>{selectedState.order === "hidden" ? (language === "zh" ? "密令" : "Sealed order") : ORDER_LABELS[selectedState.order as Order][language === "zh" ? 0 : 1]}</i>}</div></div>
