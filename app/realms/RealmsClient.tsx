...
function RealmMap({ game, language, inspectedKey, actionKey, onInspect, selectableAreaIds }: { game: Game; language: Language; inspectedKey: string; actionKey: string; onInspect: (areaId: string) => void; selectableAreaIds: string[] }) {
  ...
  return <div className={`realm-map-shell ${expanded ? "expanded" : ""} ${showAllLabels ? "show-all-labels" : "compact-labels"}`}>
    ...
    {selectedDefinition && selectedState && <section className="map-selection-dock" style={{ "--owner": selectedOwner?.color ?? "#9a8d70" } as CSSProperties}>
      ...
      <div className="map-selection-status">{selectedState.units.length > 0 && <div className="map-inspector-units">{selectedState.units.map((unit) => <MapUnitPiece key={unit.id} unit={unit} color={factionMap.get(unit.faction)?.color} language={language} />)}</div>}<div className="map-resource-chips">{selectedDefinition.castle && <i>♜ {selectedDefinition.castle}</i>}{selectedDefinition.supply && <i>▰ {selectedDefinition.supply}</i>}{selectedDefinition.power && <i>◆ {selectedDefinition.power}</i>}{selectedState.neutral && <i>⚔ {selectedState.neutral}</i>}{selectedState.garrison && <i>▣ {selectedState.garrison}</i>}{selectedState.order && <i className={selectedState.order === "hidden" ? "sealed-order-chip" : ""}>{selectedState.order === "hidden" ? (language === "zh" ? "◆ 密令" : "◆ Sealed") : ORDER_LABELS[selectedState.order as Order][language === "zh" ? 0 : 1]}</i>}</div></div>
