...
function PlanningPanel({ game, me, language, act }: { game: Game; me: Player; language: Language; act: (action: string, payload?: Record<string, unknown>) => void }) {
  ...
  if (me.submitted) return <section className="action-card waiting"><span>✓</span><h3>{text("命令已封存", "Orders locked")}</h3><p>{text("其他玩家无法看到你的命令，全部提交后同时揭晓。", "Your orders remain hidden until every faction submits.")}</p></section>;
  return <section className="action-card planning-card"><h3>{text("为每个有部队的区域下令", "Assign one order to every occupied area")}</h3><p>{text(`王庭允许 ${starLimit} 枚星级命令。`, `Court position allows ${starLimit} starred orders.`)}</p><div className="order-area-tabs">{areas.map((area) => <button key={area.key} className={selected === area.key ? "selected" : ""} onClick={() => setSelected(area.key)}>{areaName(game, area.key, language)}{orders[area.key] ? ` · ${ORDER_LABELS[orders[area.key]][language === "zh" ? 0 : 1]}` : ""}</button>)}</div><div className="order-grid">{(Object.keys(game.orderCounts) as Order[]).map((order) => { const disabled = (usedCounts[order] ?? 0) >= game.orderCounts[order] && orders[selected] !== order || order.endsWith("_star") && Object.values(orders).filter((item) => item.endsWith("_star")).length >= starLimit && orders[selected] !== order; return <button key={order} className={orders[selected] === order ? "selected" : ""} disabled={disabled} onClick={() => setOrders((current) => ({ ...current, [selected]: order }))}>{ORDER_LABELS[order][language === "zh" ? 0 : 1]} <small>{usedCounts[order] ?? 0}/{game.orderCounts[order]}</small></button>; })}</div><button className="realm-primary" disabled={Object.keys(orders).length !== areas.length} onClick={() => act("submitOrders", { orders })}>{text("封存全部命令", "Lock all orders")}</button></section>;
}
